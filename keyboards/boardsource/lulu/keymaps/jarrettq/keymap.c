...
#define LEFTER LCTL(KC_LEFT) //jumps a word to the left
#define RIGHTER LCTL(KC_RIGHT) //jumps a word to the right
#define S_LEFT LCTL(S(KC_LEFT)) //selects text blocks to the left
#define S_RIGHT LCTL(S(KC_RIGHT)) //selects text blocks to the right 
...
/* COLEMAK DH - My preferred key layout for touch typing. 
 * ,-----------------------------------------.                    ,-----------------------------------------.
...
/* GAME - Tweaked Qwerty on the left half, haven't decided what to do with the right half yet.
 * ,-----------------------------------------.                    ,-----------------------------------------.
 ...
 */
 [_GAME] = LAYOUT(
  ...
  KC_LSFT, KC_LEFT,   KC_Z,    KC_X,    KC_C, KC_LALT,  REPLAY,  AVOUT,     KC_N,    KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RSFT,
...
[_LOWER] = LAYOUT(
  ...
  _______, _______,  LEFTER,   KC_UP,  RIGHTER, KC_LPRN,                  KC_RPRN,  KC_7,    KC_8,  KC_9,  KC_F11,  KC_F12,
...
void keyboard_post_init_user(void) {
  ...
  rgblight_sethsv_noeeprom(5, 255, 130); //currently a deep orange-red
};
...
void process_layer_state_master(void) {
	switch (get_highest_layer(layer_state)) {
		case _GAME:
		  render_layer3_logo();
		  break;
...
void process_layer_state_slave(void) {
	switch (get_highest_layer(layer_state)) {
		case _GAME:
		  render_layer4_logo();
		  break;
...
bool oled_task_user(void) {
  if (is_keyboard_master()) {
      process_layer_state_master();
    } else {
      process_layer_state_slave();
  }
  return false;
}
